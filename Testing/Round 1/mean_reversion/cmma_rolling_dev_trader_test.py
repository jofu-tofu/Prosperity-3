...
from backtester.traders.cmma_rolling_dev_trader import Trader
from backtester import get_price_data

def main():
    """
    Test the CMMA Rolling Deviation Trader
    """
    print("Testing CMMA Rolling Deviation Trader")
    ...
    for i, (_, price) in enumerate(squid_vwap.items()):
        ...
        if len(price_history) >= max_lookback:
            ...
            position_increment = int(0.2 * trader.max_position)
            ...
            current_position = normalized_current + position_difference
    ...
    print("\nPosition Statistics:")
    print(f"Position Increment: {position_increment} ({0.2 * 100:.0f}% of max position)")
    ...
    # Add horizontal lines for position increments
    position_increment = int(0.2 * trader.max_position)
    for level in range(-trader.max_position, trader.max_position + 1, position_increment):
        if level == 0:
            plt.axhline(y=level, color='black', linestyle='-', alpha=0.3)
        else:
            plt.axhline(y=level, color='gray', linestyle='--', alpha=0.2)

    plt.title('Trading Positions (Increments of 0.2 * Max Position)')
