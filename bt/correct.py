...
def parse_data(data_fname: str):
    ...
    market_data = pd.read_csv(pd.compat.StringIO(market_csv), sep=";")
    trade_history = pd.read_json(pd.compat.StringIO(trade_json))
    return market_data, trade_history
...
class Backtester:
    def __init__(
        ...
        self.trader = get_trader(trader_fname)
        market_data, trade_history = parse_data(data_fname)
        ...
        self.position_limit = constants.position_limit
        self.fair_marks = constants.fair_marks
    ...
    def run(self):
        ...
        for timestamp, group in timestamp_group_md:
            ...
            self.sandbox_logs.append(
                {"sandboxLog": sandboxLog, "lambdaLog": "", "timestamp": timestamp}
            )
