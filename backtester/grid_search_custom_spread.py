...
def run_backtest(trader_path, round_day, params):
    ...
    # First, try to run with output to a file
    ...
    try:
        ...
        result = type('Result', (), {'stdout': stdout, 'returncode': 0})()

        # Clean up the output file
        os.remove(output_file)
    except Exception as e:
        print(f"Error running backtest with file output: {e}")
        # Fall back to subprocess.run
        try:
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True, encoding='utf-8')
        except UnicodeDecodeError:
            # If utf-8 fails, try with cp1252 encoding (Windows default)
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True, encoding='cp1252')

    # Extract the total profit from the output
    try:
        # Print the first 500 characters of output for debugging
        print(f"Output preview for {round_day} with params {params}:")
        print(result.stdout[:500] if result.stdout else "No output")

        # Try different regex patterns to find the profit
        profit_match = re.search(r"Total profit: ([\d,]+)", result.stdout)
        if not profit_match:
            profit_match = re.search(r"Total profit:\s*([\d,]+)", result.stdout)
        if not profit_match:
            profit_match = re.search(r"Total profit[^\d]*(\d[\d,]*)", result.stdout)
        ...
        if profit_match:
            ...
            all_numbers = re.findall(r"\b\d[\d,]*\b", result.stdout)
            if all_numbers:
                print(f"Found these numbers in output: {all_numbers[-10:]}")
            profit = 0
    except Exception as e:
        print(f"Error processing output for {round_day} with params {params}: {e}")
        profit = 0
    ...
    try:
        ...
        print(f"Error removing temporary file {temp_trader_path}: {e}")
...
def main():
    ...
    parser.add_argument('--entry', type=float, nargs='+', default=[4.0, 4.5, 5.0,5.5, 6.0],
                        help='Entry threshold values to test')
    parser.add_argument('--exit', type=float, nargs='+', default=[0.25, 0.5, 0.75, 1.0],
                        help='Exit threshold values to test')
    parser.add_argument('--entry-pay', type=float, nargs='+', default=[0.0, 1.0],
                        help='Willingness to pay values to test for entering positions')
    parser.add_argument('--exit-pay', type=float, nargs='+', default=[0.0, 1.0, 2.0],
