...
class Trader:
    ...
    Key parameters:
    - lookback: Number of periods to use for CMMA moving average calculation (default: 20)
    - vol_lookback: Number of periods to use for rolling deviation calculation (default: 50)
    ...
    def __init__(self):
        # CMMA parameters
        self.lookback = 20  # Lookback for moving average calculation
        self.vol_lookback = 50  # Lookback for rolling deviation calculation
        ...
        max_lookback = max(self.lookback, self.vol_lookback)
    ...
    def calculate_cmma(self, prices: deque) -> float:
        ...
        required_data = max(self.lookback, self.vol_lookback)
        ...
        # Calculate rolling standard deviation using vol_lookback
        if len(price_list) >= self.vol_lookback:
            # Calculate rolling returns
            returns = []
            for i in range(1, self.vol_lookback + 1):
                if i < len(price_list):
                    ret = (price_list[-i] - price_list[-i-1]) / price_list[-i-1] if price_list[-i-1] != 0 else 0
                    returns.append(ret)
            ...
            if returns:
                ...
                rolling_std = np.sqrt(variance) if variance > 0 else 1.0
            else:
                rolling_std = 1.0  # Default if we can't calculate
        else:
            rolling_std = 1.0  # Default if not enough data

        # Ensure rolling_std is not too small to avoid division by zero or extreme values
        rolling_std = max(rolling_std, 0.0001)

        # Calculate raw CMMA and normalize by rolling standard deviation
        raw_cmma = (current_price - ma) / (rolling_std * np.sqrt(self.lookback + 1))
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        for product in self.products:
            if product in state.order_depths:
                ...
                # Calculate CMMA and get the rolling standard deviation
                cmma = self.calculate_cmma(self.price_history[product])

                # Calculate rolling standard deviation for reference
                price_list = list(self.price_history[product])
                rolling_std = 1.0  # Default value
                ...
                # Store CMMA value and rolling std in trader_data
